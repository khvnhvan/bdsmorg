...
@section('docmain')
    <div class="dashboard-container">
        ...
        <br>
        <form action="{{ route('doc.store_clinic', $cl[0]->user_id) }}" method="POST"
            style="width: 85%; background-color: #F7E9E8; padding: 5%; ">
            @csrf
            <div class="clinic-info-content">
                <div class="update-donator-info">
                    <h1>ĐƠN KHÁM BÁC SĨ CỦA <p style="color: #DA4239">{{ $cl[0]->name }}</p>
                    </h1>
                    <div class="update-donator-info-form">
                        <div class="clinic-info-content-left" style="height: 550px">
                            <input type="text" name="user_id" value="{{ $cl[0]->user_id }}" hidden>
                            <select name="MaCauTL" hidden>
                                @foreach ($don as $donkham)
                                    <option value="{{ $donkham->MaCauTL }}"
                                        {{ $cl[0]->user_id == $donkham->user_id ? 'selected' : '' }}>
                                    </option>
                                @endforeach
                            </select> <br> <br>
